...
namespace Rts.Presentation
{
    ...
    public readonly struct MatchOutcome
    {
        ...
        public static MatchOutcome? Describe(MatchResult result, uint viewFactionId, long tick)
        {
            ...
            if (result.IsAgeVictory)
                return result.WinnerFactionId == viewFactionId
                    ? new MatchOutcome(UiText.T("Victory", "勝利"), UiText.T("Held the third age. ", "3つ目の時代を守り切りました。") + time)
                    : new MatchOutcome(UiText.T("Defeat", "敗北"), UiText.T("The enemy held the third age. ", "敵が3つ目の時代を守り切りました。") + time);
            return result.WinnerFactionId == viewFactionId
