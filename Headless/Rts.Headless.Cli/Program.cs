...
internal static class JsonInput
{
    ...
    private static JsonSerializerOptions Create()
    {
        ...
        o.Converters.Add(new ValueConverter<EconomyCommand>(e => new EconomyCommand(Get<uint>(e,"factionId",o), Get<ulong>(e,"issuerSequence",o),
            ...
            Get<int[]>(e,"cells",o), Get<Facing[]>(e,"facings",o), Get<Facing>(e,"facing",o), Get<EconomyPolicy>(e,"policy",o))));
    ...
    public static int Main(string[] args)
    {
        try
        {
            ...
            for(int i=1;i<args.Length;i++)
            {
                ...
                string value=key=="--allow-build-mismatch"||key=="--alloc-types"||key=="--economy"||key=="--industry"?"true":(++i<args.Length?args[i]:throw new InvalidDataException("Missing option value."));
            ...
            if(args[0]=="record")
            {
                ...
                var scenario=options.TryGetValue("--map-seed",out var mapSeed)?MapGenerator.Generate(ulong.Parse(mapSeed,System.Globalization.CultureInfo.InvariantCulture),options.ContainsKey("--economy")||options.ContainsKey("--industry"),options.ContainsKey("--industry")):JsonInput.Scenario(Required("--scenario"));
