    ...
    public static int Main(string[] args)
    {
        try
        {
            ...
            for(int i=1;i<args.Length;i++)
            {
                ...
                string value=key=="--allow-build-mismatch"||key=="--alloc-types"?"true":(++i<args.Length?args[i]:throw new InvalidDataException("Missing option value."));
