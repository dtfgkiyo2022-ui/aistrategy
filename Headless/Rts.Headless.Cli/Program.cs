    ...
    public static int Main(string[] args)
    {
        try
        {
            ...
            for(int i=1;i<args.Length;i++)
            {
                ...
                string value=key=="--allow-build-mismatch"||key=="--alloc-types"||key=="--economy"?"true":(++i<args.Length?args[i]:throw new InvalidDataException("Missing option value."));
            ...
            if(args[0]=="record")
            {
                ...
                var scenario=options.TryGetValue("--map-seed",out var mapSeed)?MapGenerator.Generate(ulong.Parse(mapSeed,System.Globalization.CultureInfo.InvariantCulture),options.ContainsKey("--economy")):JsonInput.Scenario(Required("--scenario"));
