...
毎20tick、自軍の正確な人数と観測上の推定敵人数で以下の順に配分する。敵人数にHPや隠れた軍団編成を加味しない。既定は予備200‰、維持優先。陣営ごとに同じアルゴリズムを独立実行する。

1. 自コア24 m以内に視認敵がいれば予備を迎撃へ。他軍団は人間制約が許す場合のみ、コアからの経路距離が近い順に戻す。
2. 予備目標人数はceil(生存総数×比率/1000)。偵察を除外し、予備役→コアへの経路が近い軍団→ID順に軍団全体を保持する。確保不能なら不足報告、無理に人間命令を変更しない。
3. 放棄許可のない自軍拠点へ、担当軍団を最低1軍団保持する。候補不足なら人間Priority→拠点ID順に守る。
4. 残余はFocusへ、なければ中立/敵拠点のうち推定脅威が低い→経路距離→拠点ID順へ。両拠点を所有して余剰軍がある場合は敵コアへ進軍する。未知の拠点兵力は警戒人数10として評価し、「敵10人を確認」と報告しない。
...
比較する敵プリセットは「維持型」（予備200‰、両拠点保持）と「集中型」（予備0、北Focus、南AllowAbandon）。同じ仕組みに開始時の方針を入力して作り、隠れた敵状態で分岐する特別AIは作らない。
