...
1vs1、ローカルの人間対AI／AI対AI、1マップ、歩兵・偵察・守衛（コア前の固定防御）、固定軍団、2コア・2前線拠点、簡易増援、霧、手動方針、お任せAI、擬似AI遅延、先頭からの再生を実装する。ローカル2人用には同一プロセスで操作陣営を切り替える検証モードを提供する。陣営切替時は画面・選択・報告を消してから対象陣営の情報だけを再構築する。

1週目は10人対10人、3〜4週目は合計40〜80人。100〜500人・数千人の性能保証は対象外で、5週目以降に実測する。DOTS、Jobs、Burst、物理判定、兵士ごとのUpdate、オンライン通信、Steam、実LLM、音声認識、教範検索、試合間メモリ、経済、建築、コア修復、製品用巻き戻しは実装しない。**コア前の守衛は初期配置の兵士であり、建築ではない**（生産・修復・追加建設をしないため、建築の除外と矛盾しない）。LLM・音声の独立試験は別発注とする。
...
西／東コア中心は(24,64)/(232,64)、北／南拠点は(128,96)/(128,32)。拠点は中立開始、コアHP=3000、半径4 m。拠点は破壊せず占領のみ。40人設定では各陣営20人（北歩兵8、南歩兵8、予備歩兵2、偵察2）、上限は陣営40人・合計80人。軍団は各陣営5個、編成枠は北16・南16・予備6・偵察2・守衛2、軍団IDは固定、試合中の合流・分割はしない。守衛軍団は既存IDをずらさないよう末尾に採番し、西=9・東=10とする（陣営の軍団IDは西{1,2,3,4,9}・東{5,6,7,8,10}）。
...
**守衛の初期配置**：各陣営2体を自コアの前（敵側）に置く。西は(30,60)と(30,68)、東は`256-x`の鏡像で(226,60)と(226,68)。コア中心から8 m前方の(32,60)ではなく x=30／226 とするのは、2ルートマップではx=32を含むセルが道の外（通行不可）になるためで、通行可能な最寄り列であり、かつコア中心から8 m以内（9.4の保持半径の内側）に収まる。守衛の兵士IDも既存IDをずらさないよう末尾に採番する（20人設定なら21〜24、40人設定なら41〜44）。死亡した守衛は復活しない。

| 設定 | 初期値・ルール |
|---|---|
| 歩兵 | HP100、速度2 m/s、視界20 m、射程2 m、ダメージ10、攻撃間隔20tick |
| 偵察 | HP40、速度4 m/s、視界32 m、射程2 m、ダメージ2、間隔20tick |
| 守衛 | HP800、速度**0**、視界24 m、射程8 m、ダメージ40、攻撃間隔20tick、1陣営2体。陣営人数上限に数えず、増援で補充しない |
| 拠点・コア視界 | 所有中のみ半径24 m |
...
**守衛の確定ルール（配分の対象外）**：守衛軍団（5.1、西9・東10）は0〜5のどの手順にも参加しない。

- **配分対象外**：配分の開始時に「使用済み」とし、Assignment＝コア防衛、目標＝自コアに固定する。手順1のコア防衛、手順2の予備、手順3の拠点守備、手順4の攻撃群・進撃隊のいずれにも選ばれない。手順2の予備目標人数を求める**生存総数からも除く**（動けない兵を予備の分母に入れると予備が過大になる）
- **推定敵人数に数えない**：守衛は敵から通常どおり視認でき（`VisibleEnemy`）、接触も作られる（接触が無いと敵が守衛を攻撃できない）。ただし接触に「固定防御」の印を付け、**推定敵人数の集計から除外する**。守衛軍団は集計接触（軍団単位の接触）にもしない。理由：推定敵人数は手順4の進撃判定「攻撃群の人数×2 ≥ 推定敵人数」に使われるため、コア前の守衛を数えると敵コアを目標にした攻勢が永久に進撃条件を満たさず、#49 で直した「コアを一度も攻撃しない」状態に戻る
- **人間の命令の対象外**：Focus・Retreat・Defend・Scout・MaintainReserve のいずれも守衛軍団を対象にできない。軍団を名指しした命令は実行不能（InvalidPayload）として拒否し、全軍指定の命令も守衛軍団には届かない
- **撤退しない**：手順5の劣勢撤退の判定に含めない。接敵評価の自軍人数にも数えない
- **動かないが攻撃する**：速度0のため移動段階の結果として動かない（移動処理そのものは変更しない）。コア防衛の保持（アンカー＝自コア、保持半径24 m）に入るため、自コアから24 m以内の視認敵のうち射程8 m以内のものを9.4の近接選択で攻撃する
- **追撃に入らない**：速度0の兵は9.4の追撃サイクルに入れない。追撃は「起点から12 mまで追い、任務位置の1 m以内へ帰還したら解除」という往復を前提としており、**動けない兵は帰還条件を永久に満たせず、以後いっさい標的を選ばなくなる**（実測：守衛が5428tickの間に一度も攻撃せず、射程内に敵32人がいても`TargetKind=0`のままだった）。判定は兵種ではなく「移動量が0であること」で行い、守衛に限らず速度0のユニット全般に適用する
- **占領に関与しない**：占領判定は「生存歩兵」のままなので、守衛は占領にも占領阻止にも関与しない

**守衛の仮設定**：HP800、ダメージ40、射程8 m、1陣営2体。根拠のある値ではなく出発点である。

**数値の詰めは5週目以降に回す**：HP 400／800／1600、ダメージ 20／40／80、射程 4／8／12 m、1／2／4体といった一要因ずつの比較は、4週目のうちは行わない。Ver.1の目的はAI指揮の仕組みが成立するかの検証であり、強さの調整ではない。触れる相手も遊ぶ人もいない段階で数値を詰めても、判断材料がない。4週目のうちは**合格の目安（20000tick以内の決着、Faultなし、決定論の維持）だけを関門として確認する**。強すぎれば未決着として関門に引っかかるので、破綻はそこで検出できる。
...
**共同攻勢の仮設定と比較値**：集合地点40 m手前（比較 24／40／60 m）、集合半径**8 m**（比較 8／12／16 m。#50 の比較で12 mから変更）、集合完了の割合は過半数（過半数／75%以上／全員）、移動期限の猶予200tick（100／200／400）、進撃判定の倍率×2（1.5／2／2.5）、最低維持・集合待ち・除外は各600tick（それぞれ200／600／1200、別々に変える）、接近の距離上限60 m（40／60／80 m）。

**最低維持600tickの起点**：集合の開始ではなく、**集合完了から進撃へ移った時点**から数える。集合地点までの移動が長いと、進撃を始めた直後に維持期間が満了して別目標へ乗り換え、敵と接触する前に再集合へ戻ってしまう（#50 で実測：集合完了1425tick、切替1500tick）。同じ目標での再比較では起点を更新しない。目標の取得・到達不能・人間の命令・撤退など、既存の解除条件は維持期間中でも優先する。

**記録する指標**：初回のコア被弾tick、決着tick、集合失敗の回数、実際に進撃した人数、守備に拘束された時間、同じ目標への再挑戦の回数、拠点の所有時間、**守衛が稼いだ時間**（初回のコア被弾tickから、救援軍団の先頭が自コア24 m以内に到達したtickまで）、**守衛の生存tickと撃破数**。40人対称で膠着した状態からの再開ケースも比較に含める。放棄の有利不利は、集中で突破できる条件と、留守の拠点を取られて増援源を失う条件の両方を同じ初期状態で比べる（決着の速さだけで合格にしない）。
