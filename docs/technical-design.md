...
0. **先に確定するもの**：人間の方針が有効な軍団と、撤退中・帰還後保持中の軍団を先に確定し、配分の対象から除く。残る軍団を1〜4で重複なく配分する。
...
3. 放棄許可のない自軍拠点のうち、**占領脅威がある拠点**へ担当軍団を最低1軍団保持する。候補不足なら人間Priority→拠点ID順に守る。**占領脅威のない所有拠点には守備を固定しない**（その軍団は4の攻勢に回る）。人間のDefendはこの判定に関係なく維持する。
   ...
   - **接近**：同じContactIdの連続する視認位置で、その拠点への経路距離が減った場合。不可視中の移動・敵の命令・方向変更は参照しない。期限は接近を判定した最後の観測tickから数える
   - 短期適応の警戒は守備候補の優先順位を上げるが、それ単独では最低守備を要求しない。短期適応も占領脅威（敵歩兵）の観測から更新する
4. 残余はFocusへ。なければ**軍団ごとに**攻勢目標を選ぶ。候補は中立/敵拠点と、**新規に選ぶ時点で自軍が少なくとも1拠点を所有している場合は敵コア**。
   - **比較**：「評価領域の推定敵人数×1000÷その軍団の生存人数（整数除算）」→「経路距離」→「種別（拠点、コアの順）」→ID の順で、最も小さいものを選ぶ
   - **評価領域**：その軍団の先頭の生存兵のセルから候補まで6章のA*で求めた経路について、経路セル中心を結ぶ折れ線から24 m以内（端点を含む）。各接触は一度だけ数え、個体接触と集計接触を二重に加えない。推定値は10章の上限値・情報期限に従う。未観測の拠点とコア周辺には同じ警戒仮定（10人）を適用し、観測上の不在と区別する
   - **維持**：選んだ攻勢目標は600tick維持する。同じ目標を続ける間は維持開始tickを更新しない。1〜3への転用、目標拠点の自軍取得、到達不能、劣勢撤退（5）、人間の命令で解除する
   - **再攻撃の抑制**：同じ目標から劣勢撤退または損失撤退した軍団は、その目標を600tick候補から外す
   - 未知の拠点兵力は警戒人数10として評価し、「敵10人を確認」と報告しない
5. お任せの軍団は接敵範囲24 mで自軍人数×2 < 推定敵上限の状態が40tick連続、または損失閾値到達で撤退する。帰還後60tick保持して再配分。人間Retreat完了直後も同じ60tickを設け、即時再進軍を防ぐ。

**保存する状態**：接近判定のための接触ごとの直前の位置と経路距離、占領脅威の最終判定tick、軍団ごとの攻勢目標・維持開始tick・再攻撃抑制の対象と期限は、正規状態とハッシュの対象にする（300行の「メモリに保存するのは観測イベントtickだけ」は短期適応のメモリに限る）。
...
**未決事項（次の調整で判断）**：陣営単位で攻勢の共同目標と参加軍団を決める方式、救援の到着時間と敵の接近時間を比べて守備を続けるかを決める方式。
