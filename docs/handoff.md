...
- **`LiveMatchHost` への組み込みで先に解く必要がある問題**：`CommandGateway` はプロバイダーを**1つ**しか持たず、そのプロバイダーが「人の命令の翻訳」と「自律方針」の両方を受ける。Jev をそのまま差すと**人の命令まで Jev が決め直してしまう**。要求がどちらなのかをプロバイダーが知る手段が今はない（`PolicyRequest` に印を足すのは Contracts の変更で、相方の合意が要る）。**ヘッドレスの `jev-match` は人の命令を出さないので、この問題を避けて測れる**
