...
最終更新：2026-09-18。前のセッション（別アカウントの Claude Code）から、次のセッションへ渡すためのメモです。**作業を始める前に全部読んでください。**
...
- `main` は `b2cc78f`（PR #61 のマージ）
- **PR #62（draft、未マージ）**：コア前の守衛（Sentry）。ブランチ `a/59-sentry`。**保留中**（→「4. 保留した実験」）
- ローカルのブランチは `main`・`a/59-sentry`・`a/handoff`（このメモの PR）のみ。作業用 worktree は片付け済み
...
| Issue | 内容 | 状態 |
|---|---|---|
| #29 | 4-2 敵の方針2種類と判断猶予の測定 | **次の本命**。→「5. 次にやること」 |
...
### 本命：#29 敵の方針2種類と判断猶予の測定

- 完了条件は2つ：(1)「維持型」「集中型」の敵プリセット、(2) 受付 tick を1刻みで探索し、最終成功 tick を出す
- **(1) はほぼ実装済み**：`UnityProject/Assets/RTS/Application/PolicyPresets.cs` に `maintain`（維持型）・`concentrate`（集中型）・`maintain-legacy`（改訂前の維持型、比較用）がある。完了条件を満たしているか確認するだけでよい可能性が高い
- **(2) が本題**：仕様は設計書 11 章 403 行。成功述語は「コア防衛＝撃破回避、撤退＝指定軍団の50%以上帰還、増援＝拠点陥落前の防衛到着、陽動対応＝主攻側拠点またはコア維持」。同じ命令の受付 tick を1 tick 刻みで全探索して成功 tick 集合と最終成功 tick を出す（**成功が単調とは仮定しない**）。初観測から最終成功までを猶予とし、理解・入力時間として追加 60 tick を入れたケースも比べる。結果なしは「猶予なし」、上限まで判定できない場合は「未評価」
- **判断猶予の測定コードはまだ無い**（2026-09-18 に `判断猶予`・`Grace`・`grace` で `UnityProject/Assets/RTS` と `Headless` の `.cs` を検索して該当なし）。新規に作る。置き場所は、既存の `analyze`（`Headless/Rts.Headless.Cli/AnalyzeCommand.cs`）と同じく CLI のコマンドにするのが自然
