...
## ★★ 2026-09-23 昼、再びアカウント上限のため引き継ぎ ★★

**このアカウントも利用上限に達したため、また別アカウントへ引き継ぐ。パソコンは同じ。作業中の変更はすべて main にマージ済みで、作業ツリーはきれい（Unity生成の未追跡ファイル2つを除く。コミットしない）。**

- `main` の先頭：`b01b31a`（PR #173、Codexモデル使い分けの更新）。`git log --oneline -3 origin/main` で確認
- **開いている PR は #62（守衛、draft、保留中）だけ**。他はすべてマージ済み
- **Codex は 11:38 の復帰予定を過ぎ、実際に使えることを確認済み**（`gpt-5.6-luna` で応答テスト成功）
- **次にやること：#18 交易路の発注はまだ実行していない。** 発注文は `D:/rts-verify/astra/request_v3-5-18-trade-route_luna.md` に用意済み（内容は下の「V3-5」節を参照）。`gpt-5.6-luna` を `model_reasoning_effort=max` で使い、独立プロセスで走らせて完了を待つこと（長時間かかる想定）
- **モデル使い分けが今日昼に更新された**（オーナー指示、下の「0.」の節にも反映済み）：実装は常に Codex へ。Opus で難しい・大きい内容は luna + effort max、普通の作業は既定effortのluna、それでも無理なら astra
- 前のアカウントのメモリ（Cドライブの Claude Code の projects 配下の memory フォルダ）は今回も自動では引き継がれない。**自分から読みに行くこと**（`MEMORY.md` が索引）

---

## 0. アカウントを引き継いだ人がまず読む要約（2026-09-25 更新）

**いまは Ver.3 の V3-5（文明の中の内容を幅広く足す段階）を作っている。** 設計は `docs/technical-design-v3.md` の **32章**（#1〜#22まで実装記録がある。32.1 に作り方の決まりごと）。V3-1〜V3-4 は完了済み。

- `main` の先頭：PR #180（僧侶・改宗、#22）までマージ済み。`git log --oneline -3 origin/main` で確認。**開いているPRは無いはず**
- 全体テストは 773 件（`dotnet test Headless/Rts.Headless.slnx --configuration Release`、約18分）。**出力は UTF-8 のファイルに落として読む**。CI待ち・マージは PowerShell の `Start-Process`（独立プロセス）で行う（Bashの背景実行はセッションが切れると消える。メモリ `detached-processes-on-windows.md`）
- 報告・合間の一言は**すべて日本語**（メモリ `always-japanese-output.md`）
- **測定テストは短く・毎 tick 見る**（メモリ `measurement-tests-tick-by-tick.md`）。`Assume` 待ちのテストは条件が揃わないと NUnit で「スキップ」になり、合計に出ず「成功!」と表示されるので**通ったと誤認しやすい**
- **Codex のモデル使い分け**（メモリ `codex-model-selection.md`）：実装は常に Codex へ発注し、Claude はレビューと検証に徹する。既定は `gpt-5.6-luna`（effort medium）、広い/決定論に関わる機能は high、詰まったら `gpt-6-astra`。`-m` は必ず明示する
- **設計の芯に関わる決定はAstraにレビュー依頼**（メモリ `astra-review-core-design.md`）。依頼文は `D:/rts-verify/astra/request_*.md`、結果は `result_*.md`
- **Codex実行のバックグラウンド化の注意**：`run_in_background: true` と自前の末尾`&`を併用すると、ツール呼び出しが返った時点でプロセスが打ち切られ応答が得られない（2026-09-25に発生）。`run_in_background`だけを使い、コマンド自体に`&`を付けないこと

### V3-5 の進み具合（32章、#は技術設計 32章の表の番号）

| # | 中身 | 状態 |
|---|---|---|
| #1〜#17 | 斥候〜城（3つ目の時代の大型拠点）まで | 完了（PR #153〜#171） |
| #18 | 交易路 | 完了（PR #175） |
| #19 | 宝石 | 完了（PR #176） |
| #20 | 傭兵 | 完了（PR #177） |
| #21 | 時代到達による勝利（既定オフの任意の勝ち筋） | 完了（PR #179） |
| #22 | 僧侶・改宗（FactionIdを書き換えず死亡→新規生成方式、既定オフ） | 完了（PR #180） |

釣り合いの調査（冶金が弱い件）は32.23に記録。見送り済み（変更は取り消し済み）。

### ★ 次にやること（未着手、実装前に検討）

- **勝利条件の追加**：決着判定（`ResolveVictory`）に触れるため要調査
- **資源の追加**：mapgen-3 に手を入れる必要がある

### 分かっている課題（数の調整。資料 32.11・32.14・32.23 に記録。まだ触らない）

- 冶金が8種中1種でしか勝てない（食料と時間が詰まりやすい）。根本原因は特定済み（32.23）：村人が「待機」にならず`AssignWork`の再割り当てがほぼ働かない。本当に直すには待機を待たない割り込みが要る（今回は見送り）
