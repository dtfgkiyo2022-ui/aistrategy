...
- **実際の Jev に1回だけ送った結果**（`[Explicit]` の `LiveSmokeOneRealRequest`、`PROBE_KEY` が要る）：3.3秒、入力609トークン、`focus=south`・**確信度 0.25**・撤退確率 0.38。**気になる点：確信度が 0.25 と低く、既定の閾値 0.7 ではほぼ命令が出ない可能性がある**。#87 では「確信度は戦況に追従」としか見ておらず、`confidence` が何の値か（選択肢の最大確率とは限らない：3択で 0.25 は 1/3 を下回る）は確かめていない。閾値の較正が先に要る。1回の測定で結論は出さない
