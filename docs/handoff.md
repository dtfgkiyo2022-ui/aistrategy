...
## 0. アカウントを引き継いだ人がまず読む要約（2026-09-21）

**リポジトリはきれいな状態で渡しています。** 作業ツリーに未コミットの変更はなく、進めていた PR はすべてマージ済みです。

- `main` の先頭は **PR #123**（Ver.2：Jev の組み込み）。`git log --oneline -3 origin/main` で確認する
- **開いている PR は #62（守衛、draft、保留中）だけ**。5週目以降に再検討すると決めたもので、触らなくてよい
- 全体テストは **481件すべて成功**（`dotnet test Headless/Rts.Headless.slnx --configuration Release`、15〜35分）
...
### いま進行中の仕事：Ver.2（外部の判定モデル Jev の組み込み）

段階5（100〜500体）の性能確認まで終わり、**Ver.2 に入っています**。Jev は「どこが勝敗を分けるか」を答え、その答えを命令に変える仕組みが**ヘッドレスでは動いています**（`jev-match` コマンド）。詳細は下の 09-21 の節を読んでください。

**次にやる候補（オーナーと相談して決める）**

1. **ほかのシナリオ・陣営で事実の正答率が保たれるか確かめる**（$0.01 程度、A だけで可能）
2. **Jev あり・なしで勝敗を比べる**（命令が役に立っているかの最初の証拠）
3. **ゲーム本体（`LiveMatchHost`）への組み込み**。`CommandGateway` がプロバイダーを1つしか持たず、**人の命令まで Jev が決め直してしまう**問題を先に解く必要がある。`PolicyRequest` に印を足すのは Contracts の変更で、相方の合意が要る（CLAUDE.md ルール7）
