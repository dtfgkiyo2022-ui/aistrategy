...
- **オーナーが後で確認する項目（2026-09-21 オーナー申告）**：480体の画面の見やすさ（`CaptureCrowd` の画像、または実際に再生して）、購入素材の見た目の続き、試遊の残り（敵の残像・推定兵力、陣営視点の切り替え、決着までの長時間、`concentrate` 相手）。オーナーが確認するまで、私からはこれらの評価を進めない。実戦的な大規模配置（陣形、軍団ごとの人数）の設計も、見やすさを見てからオーナーが決める
