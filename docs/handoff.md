...
## 0. アカウントを引き継いだ人がまず読む要約（2026-09-22 夜 更新）

**いまは Ver.3（内政と文明）を作っている。** 設計は `docs/technical-design-v3.md`、進め方は `ROADMAP.md`（V3-1〜V3-4）。企画書は 2026-09-22 に改訂済み（内政と戦闘、原始時代から文明に枝分かれ、Mindustry・Factorio 風のライン）。

- `main` の先頭は `git log --oneline -3 origin/main` で確認する。**開いている PR は #62（守衛、draft、保留中）だけ**のはず
- 全体テストは約 690 件（`dotnet test Headless/Rts.Headless.slnx --configuration Release`、10〜15分）。**出力は UTF-8 のファイルに落として読む**（`D:/rts-verify/fulltest4.ps1` を `Start-Process` で起動し、`D:/rts-verify/full4.log` の `EXITCODE=` を待つ）
...
| 段階 | 中身 | 状態 |
|---|---|---|
| V3-1 最小の内政 | ランダムマップ、食料・木材、村人、兵舎、歩兵の生産 | 完了（PR #136〜#142） |
| V3-2 加工とライン | 鉱石・金属、ベルト、採掘場・精錬所、手運び、お任せがラインを組む、画面 | 完了（PR #143〜#146）。ラインは手運びの2〜3倍速い |
| V3-3 内政のAIと方針 | 人が触った物からAIが手を引く、お任せに戻す、方針3つ（均衡・兵・内政）、画面 | 完了（PR #148〜#150） |
| （試遊の要望） | 軍団をドラッグで囲んでまとめて選ぶ | 完了（PR #147） |
| V3-4 原始時代と文明 | 設計（25〜31章、オーナー判断を反映）、PR1 地形（森・川・山、mapgen-3） | 設計は PR #151、地形は PR #152 |

### ★ 次にやること：V3-4 の PR2〜5（設計書 30章の表）

オーナーの判断（2026-09-22）：
- 文明は推奨どおり：**農耕の文明**（農場で食料、人口+20、兵は普通で多い）／**冶金の文明**（採掘場・精錬所で金属、鍛えた歩兵 HP140・攻撃13）
- **枝分かれはまず1回**（原始 → 文明）。最終形ではない。文明の中の時代や3本目の文明は V3-5 以降。**足せる形にしておく**
- **地形も作る**（通れない地形の種類分けまで。移動の速さは変えない）

残り：
1. PR2：原始時代と「時代を進める」操作（コアで、食料400・木材300・1200 tick）、文明の記録
2. PR3：2つの文明の違い（農場、人口+20、鍛えた歩兵）
3. PR4：お任せの文明選び（周りの鉱石と食料の点を数える）と農耕のライン。関門の測定
4. PR5：画面（時代を進めるボタン、文明の選択、農場の表示）

### 作り方の決まりごと（この段階で固まったもの）

- 新しい仕組みは**旗で切り替え**、古いマップ（Ver.1・mapgen-1・mapgen-2）のシナリオのバイト列・正規状態・挙動を変えない（例：`EconomyRules.Industry`、地形は schema 5 のときだけ、人の預かりと方針は Industry のときだけ）
- ScenarioBinary の版：1〜3（V3-1 まで）、4（ライン）、5（地形）。**版を足したら `TuningRuleTests` の「まだ無い版」の番号も上げる**（2回踏んだ）
- 表示側のコードは Unity を閉じなくても確かめられる：スクラッチの `prescheck/PresCheck.csproj`（Presentation と Contracts を Unity の DLL で `dotnet build`）。無ければ、`netstandard2.1`・`UnityEngine/*.dll` 参照で作り直す
- Contracts を変えたら **Issue #135 に告知を追記**（返事は待たない）
- 実装は Codex に発注する決まりだが、この日は Claude が自分で実装した（Codex の利用上限が 2026-09-23 11:38 まで）

### 測った数（お任せ同士、8種、36000 tick）

- mapgen-2（ライン）：5つ決着（種1・2・4・6・8）。方針を出さない試合は V3-3 でも全く同じ
- mapgen-3（地形）：6つ決着（種1・2・3・4・7・8）
- 未決着の種の理由は調べていない（測って報告で止める方針）

### 前のアカウントのメモリ（自動では引き継がれません）

Claude Code のメモリはアカウントに紐づくので、**新しいアカウントには引き継がれません。** 前のアカウントのメモリはただのテキストファイルとして残っているので、**最初に一式読んでください**。場所は `C:/Users/きよ/.claude/projects/D----------------aistrategy-aistrategy/memory/` で、`MEMORY.md` が索引です。
...
| `codex-implements-claude-reviews.md` | 実装は Codex に発注、Claude はレビュー（ただし下の「2.」参照） |
