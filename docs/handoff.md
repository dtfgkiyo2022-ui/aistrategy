...
- **実際の Jev に1回だけ送った結果**（`[Explicit]` の `LiveSmokeOneRealRequest`、`PROBE_KEY` が要る）：3.3秒、入力609トークン、`focus=south`・**確信度 0.25**・撤退確率 0.38。- **確信度の読み（訂正済み）**：0.25 は、試験に使った状態が拮抗するよう作ったものだったため。#87 の実測では確信度は 0.12〜0.86 で戦況に追従し、敵が約30体見える明確な戦況では 0.77〜0.86、曖昧な戦況では 0.12〜0.25。**既定の閾値 0.7 は、明確な戦況では超え、曖昧なら超えない**位置にある（前の版に「ほとんど命令が出ない」と書いたのは言い過ぎ）。較正は未了：0.7 が適切か、`confidence` が何の値か（3択で 0.12〜0.25 は 1/3 を下回る）は確かめていない
